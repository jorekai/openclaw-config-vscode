...
describe("schema security policy", () => {
  it("normalizes policy values", () => {
    const normalized = normalizePolicyInput({
      ...
      allowedRepositories: [" NilsJorek/OpenClaw-Config-Vscode ", "nilsjorek/openclaw-config-vscode"],
    });

    assert.deepEqual(normalized.allowedHosts, ["raw.githubusercontent.com"]);
    assert.deepEqual(normalized.allowedRepositories, ["nilsjorek/openclaw-config-vscode"]);
  });

  it("accepts https URL with allowlisted host and repository", () => {
    const evaluation = evaluateUrlSecurity(
      "https://raw.githubusercontent.com/nilsjorek/openclaw-config-vscode/main/schemas/live/manifest.json",
      {
        requireHttps: true,
    ...
    assert.equal(evaluation.repository, "nilsjorek/openclaw-config-vscode");
  });
  ...
  it("extracts repository from URL path", () => {
    const repository = extractRepositoryFromUrl(
      new URL("https://raw.githubusercontent.com/nilsjorek/openclaw-config-vscode/main/schemas/live/manifest.json"),
    );
    assert.equal(repository, "nilsjorek/openclaw-config-vscode");
